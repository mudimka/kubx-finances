...
class DataProcessor
{
    ...
    public static function processDeliveryDates($date)
    {
        $result = null;

        if (!empty($date)) {
            if (str_contains($date, '-')) {
                $delivDays = explode('-', $date);
                $dateFrom = (new \DateTime())->modify("+ {$delivDays[0]} day");
                $dateTo = (new \DateTime())->modify("+ {$delivDays[1]} day");

                $result = [
                    'value_from' => $dateFrom->format('j'),
                    'value_to' => $dateTo->format('j'),
                ];
                if ($dateFrom->format('m') !== $dateTo->format('m')) {
                    $result['month_from'] = DataProcessor::getMonthNameForDate($dateFrom->format('m'));
                    $result['month_to'] = DataProcessor::getMonthNameForDate($dateTo->format('m'));
                } else {
                    $result['month'] = DataProcessor::getMonthNameForDate($dateFrom->format('m'));
                }
            } else {
                $dateVal = (new \DateTime())->modify("+ {$date} day");
                $result = [
                    'value' => $dateVal->format('j'),
                    'month' => DataProcessor::getMonthNameForDate($dateVal->format('m'))
                ];
            }
        }
        return $result;
    ...
    public static function sortResultByCodes($result, $codes, $is_object = false)
    {
        $newResult = [];
        ...
        if ($is_object) {
            foreach ($codes as $code) {
                if ($result[$code]) {
                    $newResult[] = $result[$code];
                }
            }
        } else {
            foreach ($codes as $code) {
                $index = array_search($code, array_column($result, 'code'));
